    ...
    <nav className="fixed top-0 left-0 right-0 z-50 h-16 w-full border-b border-border/40 bg-background/80 backdrop-blur-md px-4 sm:px-6 lg:px-8 shadow-xs shadow-black/5 select-none">
      <div className="mx-auto flex h-full max-w-6xl items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 pl-2">
          ...
          <DropdownMenu>
            ...
            >
              <div className="px-2.5 py-2 flex flex-col gap-0.5 select-none pointer-events-none mb-1">
                ...
                <span className="text-[10px] font-mono font-medium text-muted-foreground truncate block">
                  {authenticatedUser?.email}
                </span>
              </div>
              
              <div className="h-[1px] bg-border/60 my-1 w-full" />
              
              <DropdownMenuItem onClick={() => navigate({ to: '/about' })} className="flex items-center gap-2 px-2.5 py-2 text-xs font-bold text-foreground rounded-lg cursor-pointer transition-colors focus:bg-muted">
              ...
              <DropdownMenuItem onClick={handleDropdownLogout} className="flex items-center gap-2 px-2.5 py-2 text-xs font-black text-destructive hover:text-destructive rounded-lg cursor-pointer transition-colors focus:bg-destructive/10">
          ...
          <Button
            ...
            className="flex md:hidden h-9 w-9 rounded-xl text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors focus:outline-none cursor-pointer"
          >
      ...
      >
        ...
        <Link 
          ...
          className="flex items-center gap-3 p-3 rounded-xl text-sm font-black text-foreground hover:bg-muted/60 transition-all [&.active]:text-primary [&.active]:bg-primary/10"
        >
        ...
        <button 
          onClick={() => triggerPremiumGate('Document Transpiler (Docs)')}
          className="w-full flex items-center justify-between p-3 rounded-xl text-sm font-black text-foreground hover:bg-muted/60 transition-all text-left focus:outline-none cursor-pointer"
        >
          <span className="flex items-center gap-3">
            <BookOpen className="h-4 w-4 shrink-0 text-muted-foreground" />
            <span>Document Transpiler</span>
          </span>
          <Badge className="bg-amber-500/10 text-amber-500 border border-amber-500/20 px-2 py-0 rounded text-[9px] font-extrabold uppercase scale-90 tracking-wide"><Crown className="h-2.5 w-2.5 mr-0.5 inline fill-amber-500/10" />Pro</Badge>
        </button>
              ...
              {modalStatus === 'sent' ? (
                ...
                <form onSubmit={handleDemoSubmit} className="space-y-4" autoComplete="off" noValidate>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-extrabold uppercase tracking-widest text-muted-foreground flex items-center gap-1.5"><Mail className="h-3.5 w-3.5" /> Corporate Email</label>
                    <input 
                      type="email" 
                      required
                      ...
                      className="w-full h-10 px-3 rounded-lg border border-border bg-background text-xs shadow-inner focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-all text-foreground font-medium"
                    />
                  </div>

                  <div className="space-y-1.5">
                    <label className="text-[10px] font-extrabold uppercase tracking-widest text-muted-foreground flex items-center gap-1.5"><MessageSquare className="h-3.5 w-3.5" /> Core Target Scale Objectives</label>
                    <textarea 
                      required
                      rows={3}
                      name="sf_portal_msg"
                      ...
                      className="w-full p-3 rounded-lg border border-border bg-background text-xs shadow-inner resize-none focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-all text-foreground leading-relaxed font-medium"
                    />
                  </div>
